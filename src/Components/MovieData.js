


 export const MovieData = [
    {
      id: "1",
      Title: "Nobody",
      Year: "2021",
      description:
        "While walking late one evening, ordinary man Hutch Mansell (Bob Odenkirk) witnesses an incident of street harassment that continues to escalate in danger for the young female victim. Hutch then takes it upon himself to intervene and aid the distressed woman. Yet unbeknownst to him, his actions put him on a dangerous path of vengeance with a notorious drug lord as Hutch stood between him and his schemes. Directed by Ilya Naishuller. Connie Nielsen and Christopher Lloyd co-star.",
      Type: "movie",
      PURL:
        "https://fr.web.img6.acsta.net/c_310_420/pictures/21/01/04/17/52/0328539.jpg",
      rate: "4",
      link: "https://www.youtube.com/watch?v=wZti8QKBWPo&ab_channel=UniversalPictures",
    },
    {
      id: "2",
      Title: "Tina",
      Year: "2021",
      description:
        "This documentary provides an intimate look at the life and career of iconic American-born singer Tina Turner. It explores her early life in Brownsville, Tennessee, her unexpected rise to fame in the 1960s, the upheavals in her personal and professional life, and her unexpected career resurgence and superstardom in the 1980s. Directed and written by Daniel Lindsay and T.J. Martin.",
      Type: "movie",
      PURL:
        "https://m.media-amazon.com/images/M/MV5BYmJjODZlYjItZTE2Zi00YjI5LThmNTAtNzQ0OGMyNjAzOTIzXkEyXkFqcGdeQXVyMTAyMjQ3NzQ1._V1_UY1200_CR90,0,630,1200_AL_.jpg",
        rate: "3",
      link: "https://www.youtube.com/watch?v=w9IkVtLvflU&ab_channel=HBO",
    },
    {
      id: "3",
      Title: "Thunder Force ",
      Year: "2021",
      description:
        "Two childhood friends become an unlikely crime-fighting superhero duo after creating a formula that gives ordinary people superpowers. Directed by Ben Falcone.",
      Type: "movie",
      PURL: "https://fr.web.img6.acsta.net/pictures/21/03/23/10/41/1129711.jpg",
      rate: "3",
      link: "https://www.youtube.com/watch?v=qnx6-YLXFwg&ab_channel=Netflix",
    },
  
    {
      id: "4",
      Title: "Barb and Star Go to Vista Del Mar ",
      Year: "2021",
      description:
        "In their lifetime of friendship, kindred spirits Star (Kristen Wiig) and Barb (Annie Mumolo) remained at one another's side through the challenges and triumphs of life. Yet as middle-age approaches, both women feel the inexplicable urge to explore the world. Armed with their signature exuberance, Barb and Star leave their rural Midwestern town behind to embark on a holiday, their first beyond the confines of their community. Directed by Josh Greenbaum; co-starring Jamie Dornan and Damon Wayans Jr.",
      Type: "movie",
      PURL: "https://image.tmdb.org/t/p/w300/cq0iE4niwJCxDrduaOVxbMDcbot.jpg",
      rate: "3",
      link: "https://www.youtube.com/watch?v=yvmDgXfYPbA&ab_channel=MovieclipsTrailers",
    },
    {
      id: "5",
      Title: "Soul ",
      Year: "2020",
      description:
        "This animated film features the voice of Jamie Foxx as Joe Gardner, an average guy teaching music in middle school whose real dream is to be a jazz musician. When he finally earns the chance to perform onstage, he has an accident that leaves him in a near-death state. As his soul is scheduled to proceed to the afterlife, he discovers that his body is still not dead. He escapes to a beforelife place where he tries to negotiate a deal so he could return to Earth—before it’s too late. Directed by Pete Docter. Featuring the voices of Tina Fey and Angela Bassett.",
      Type: "movie",
      PURL: "https://fr.web.img6.acsta.net/pictures/20/10/15/11/09/5088324.jpg",
      rate: "5",
      link: "https://www.youtube.com/watch?v=xOsLIiBStEs&ab_channel=Pixar",
    },
    {
      id: "6",
      Title: "The Reckoning",
      Year: "2020",
      description:
        "In an isolated rural community, a young woman named Grace (Charlotte Kirk) reels in the wake of her husband Joseph's (Joe Anderson) death as he unexpectedly took his own life. Facing the unwanted advances of her crooked landlord, the grieving Grace rebukes him, only to find herself thrust into vehement accusations of witchcraft by the spurned man. Yet what her neighbors do not know is just how far she will go to seek justice. Directed by Neil Marshall. Co-starring Steven Waddington, Sean Pertwee, and Ian Whyte.",
      Type: "movie",
      PURL:
        "https://m.media-amazon.com/images/M/MV5BNDdlZjA3MzgtNmJlNC00NGZkLWFhNGMtNTk4ZmQwMDM1YzY0XkEyXkFqcGdeQXVyMDM2NDM2MQ@@._V1_.jpg",
        rate: "2",
      link: "https://www.youtube.com/watch?v=6wAdEY10flE&ab_channel=MovieTrailersSource",
    },
    {
      id: "7",
      Title: "Let Him Go ",
      Year: "2020",
      description:
        "Based on the novel by Larry Watson, the film focuses on the story of retired sheriff George Blackledge (Kevin Costner) whose wife Margaret (Diane Lane) is bent on finding their grandson after the death of their son. Their daughter-in-law Lorna (Kayli Carter) has taken the boy to go live with another man. When the Blackledges discover that their grandson’s new family is involved in dodgy situations, they try to rescue him, but not without becoming entangled in this new family’s problems. Written and directed by Thomas Bezucha.",
      Type: "movie",
      PURL:
        "https://encrypted-tbn2.gstatic.com/images?q=tbn:ANd9GcRDP7YFWTu86mTLyQHJSTRkuTHLB_Ky_EY5uKMQ9ZDxeSpdZ8BZ",
        rate: "4",
      link: "https://www.youtube.com/watch?v=bE8pwEF-3TI&ab_channel=FocusFeatures",
    },
    {
      id: "8",
      Title: "Promising Young Woman ",
      Year: "2020",
      description:
        "Cassie's (Carey Mulligan) future once seemed bright and full of promise until one night, a chance encounter thwarted her plans, leaving her to pick up the pieces of her life. The medical school drop-out spends her days working at a coffee shop but by night she takes on unexpected work, seeking justice to cope with her past. Written and directed by Emerald Fennell, the film co-stars Bo Burnham, Laverne Cox, and Alison Brie.",
      Type: "movie",
      PURL:
        "https://m.media-amazon.com/images/M/MV5BZThjNDhkNWQtYzIyZC00MDcxLWFlYjctODhhMmM4YzdiZmI3XkEyXkFqcGdeQXVyMDA4NzMyOA@@._V1_.jpg",
        rate: "3",
      link: "https://www.youtube.com/watch?v=7i5kiFDunk8&ab_channel=FocusFeatures",
    },
    { id: "9",
      Title: "Sound of Metal",
      Year: "2019",
      description:
        "Directed by Darius Marder, this film is about a former drug addict named Ruben (Riz Ahmed) who’s an active drummer in a musical duo called Blackgammon. He performs through life with his loving girlfriend and bandmate Lou (Olivia Cooke). However, things change when Ruben suddenly feels some form of hearing loss. As the medical situation threatens to transition from being a temporary loss to a permanent one, Ruben and Lou try to deal with the situation as best they can. Lauren Ridloff and Mathieu Amalric co-star.",
      Type: "movie",
      PURL:
        "https://m.media-amazon.com/images/M/MV5BNjcyYjg0M2ItMzMyZS00NmM1LTlhZDMtN2MxN2RhNWY4YTkwXkEyXkFqcGdeQXVyNjY1MTg4Mzc@._V1_UY1200_CR90,0,630,1200_AL_.jpg",
        rate: "4",
      link: "https://www.youtube.com/watch?v=VFOrGkAvjAE&ab_channel=AmazonPrimeVideo",
    },
  ];




export default MovieData;